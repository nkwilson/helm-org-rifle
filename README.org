...
+  Searches can now be sorted by either the latest timestamp in each node or by the order that nodes appear in their buffers.  To change the sort order, run a command with a universal prefix (=C-u=).  The sort order remains after setting it, and the defaults may be customized.
