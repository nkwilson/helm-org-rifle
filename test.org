...
#+BEGIN_SRC elisp
  (helm :sources (helm-build-in-file-source "test" "~/org/inbox.org"
                                            :search 'org-entry-re-search-forward
:get-line 'helm-org-rifle-get-entry
:before-init-hook (lambda () (when (org-before-first-heading-p)
...
#+BEGIN_SRC elisp
  ...
  (defun org-entry-re-search-forward (pattern)
    "Move point to the beginning of the next entry that matches PATTERN."
    (outline-next-heading)
    (while (not (org-entry-matches-p pattern))
      (outline-next-heading)))
  ...
  (defun org-get-next-matching-entry (pattern)
    "Return (CONTENTS . BEGINNING-POINT) for the next entry that matches PATTERN."
    (when (org-entry-re-search-forward pattern)
      (cons (org-get-entry-contents) (point))))
  
  (defun helm-org-rifle-get-entry (&rest args)
    "Return (CONTENTS . BEGINNING-POINT) for the current entry."
    (cons (org-get-entry-contents) (point)))
  
  (defun org-get-entry-contents (&optional point)
    "Get contents of entry (not subtrees) at POINT or current point."
    (buffer-substring-no-properties (org-entry-beginning-position) (org-entry-end-position)))
  
  (defun org-get-matching-entries (pattern)
    "Return list of contents of entries that match PATTERN."
    (save-excursion
      (save-restriction
...
(outline-next-heading))
        ...
        (helm-org-rifle-accumulate 'org-get-next-matching-entry pattern))))
  
  (org-entry-re-search-forward "blueberry")
  (org-get-matching-entries "blueberry")
  ...
  (acc 'org-entry-get-next-matching-entry "blueberry")
#+END_SRC
