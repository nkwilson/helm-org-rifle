...
[2016-03-31 Thu 01:07] Well, the new code works and is a tiny bit faster!  Except for when it goes into an infinite loop.  It doesn't happen every time though; it depends on the query and the file that's being searched.  Right now it happens when I search =main.org= and =inbox.org= for =emacs=.  Probably something to do with not advancing to the next heading properly, or not saving the search position correctly.  Maybe that little optimization I did isn't worth it...well, that one doesn't seem to matter.  Anyway I'll have to step through it...
