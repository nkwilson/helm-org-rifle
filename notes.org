...
(defmacro test-regexps (targets-tokens regexps)
`(append '(("Num" "Expect" "Result" "Token" "Target"))
          (cl-loop for rel in ,regexps
                   for prefix = (nth 1 rel)
                   for suffix = (nth 2 rel)
                   append (cl-loop for set in ,targets-tokens
                                   append (cl-loop for token in (cddr set)
                                                   for target = (car set)
                                                   for expect = (cadr set) collect
                                                   (list (car rel)
                                                         (format "%s" expect)
  ...
  ;; Test against these strings to see if substring matching works properly
  ;; Format: (target expect-t-or-nil tokens)
  (setq substringtestlist '(
                            ("videos" nil "eos")
                            ("symbol-name" t "name")
  ...
  ;; Regexp test list: list of lists in format:
  ...
  (append '(("Num" "Expect" "Result" "Token" "Target"))
          (cl-loop for rel in regexptestlist
                   for prefix = (nth 1 rel)
                   for suffix = (nth 2 rel)
                   append (cl-loop for set in substringtestlist
                                   append (cl-loop for token in (cddr set)
                                                   for target = (car set)
                                                   for expect = (cadr set) collect
                                                   (list (car rel)
...
#7 almost works, but there's the problem: it doesn't match tags correctly.

Just for fun, macro-style:

#+BEGIN_SRC elisp
  (defmacro test-regexps (targets-tokens regexps &optional show-regexps)
  ...
  When SHOW-REGEXPS is non-nil, display the PREFIX and SUFFIX."
    `(append '(("Num" "Expect" "Result" "Token" "Target" ,(if show-regexps '("Prefix" "Suffix") "")))
             (cl-loop for rel in ,regexps
                      for prefix = (nth 1 rel)
                      for suffix = (nth 2 rel)
                      append (cl-loop for set in ,targets-tokens
